...
let lastWeeklyDigestGenerated = null;
...
app.get('/api/digest/weekly', async (req, res) => {
    try {
        // Check if we have a cached digest that's less than 24 hours old
        if (weeklyDigest && lastWeeklyDigestGenerated) {
            const hoursSinceLastDigest = (Date.now() - new Date(lastWeeklyDigestGenerated).getTime()) / (1000 * 60 * 60);
            if (hoursSinceLastDigest < 24) {
                console.log('Returning cached weekly digest');
                return res.json(weeklyDigest);
            }
        }
        ...
        weeklyDigest = digest;
        lastWeeklyDigestGenerated = digest.generated;
...
// Backward compatibility - old digest endpoint defaults to weekly
app.get('/api/digest', async (req, res) => {
    req.url = '/api/digest/weekly';
